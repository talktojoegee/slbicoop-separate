...
class PaymentController extends BaseController
{
    ...
    public function __construct(){
        ...
        //edit and delete only applies to detail if that code has not being used in the gl table then should be enabled edit and delete.
    }
...
//
    ...
    public function postThirdpartyPaymentEntry(){
        ...
        if($_POST){
          ...
          if($source == 1){
            ...
            $vendor = $this->vendor->getVendorById($this->request->getVar('vendor'));
            $data = [
              'entry_payment_date' => $this->request->getVar('payable_date'),
              'entry_bank_id'=>$vendor->v_bank_id,
              'entry_amount'=>(float)str_replace(',', '', $this->request->getVar('amount')),
              'entry_gl_account_no'=>$vendor->v_gl_code,
              'entry_reference_no'=>$this->request->getVar('reference_no'),
              'entry_narration'=>$this->request->getVar('narration'),
              'entry_payee_name'=>$vendor->v_company_name,
              'entry_payee_bank'=>$vendor->v_bank_id,
              'entry_bank_account_no'=>$vendor->v_bank_account,
              'entry_sort_code'=>$vendor->v_sort_code,
              'entry_attachment'=>$filename,
              'cart' => 0
            ];
            $this->thirpartypaymententry->save($data);
    ...
    public function setupVendor(){
      ...
      $username = $this->session->user_username;
      $this->authenticate_user($username, 'pages/payment/setup-vendor', $data);
    }

  public function postVendorSetup(){
    helper(['form']);
    $data = [];
      $rules = [
        "companyName"=>[
        ...
        "companyPhoneNo"=>[
          "rules"=>"required",
          "errors"=>[
            "required"=>"Enter a functional company phone number"
          ]
        ],
        "glCode"=>[
          "rules"=>"required",
          "errors"=>[
            "required"=>"Assign a GL account to this vendor"
          ]
        ],
        "bank"=>[
          "rules"=>"required",
        ...
        "emailAddress"=>[
          ...
          "errors"=>[
            ...
            "valid_email"=>"Enter a valid mail address",
          ]
        ],
      ];
    ...
    if($_POST){
      if($this->validate($rules)){
        $data = [
          'v_company_name'=>$this->request->getVar('companyName'),
          'v_company_email'=>$this->request->getVar('companyEmail'),
          'v_phone_no'=>$this->request->getVar('companyPhoneNo'),
          'v_website'=>$this->request->getVar('website') ?? null,
          'v_gl_code'=>$this->request->getVar('glCode') ?? null,
          'v_bank_id'=>$this->request->getVar('bank') ?? null,
          'v_bank_account'=>$this->request->getVar('bankAccountNo') ?? null,
          'v_sort_code'=>$this->request->getVar('sortCode') ?? null,
          'v_address'=>$this->request->getVar('address') ?? null,
          'v_contact_first_name'=>$this->request->getVar('firstName') ?? null,
          'v_contact_last_name'=>$this->request->getVar('lastName') ?? null,
          'v_contact_phone_no'=>$this->request->getVar('phoneNo') ?? null,
          'v_contact_email'=>$this->request->getVar('emailAddress') ?? null,
          'v_contact_position'=>$this->request->getVar('position') ?? null,
          'v_contact_description'=>$this->request->getVar('description') ?? null,
          'created_by'=>1,
          'created_at'=>date('Y-m-d')
        ...
        $alert = array(
          'msg' => 'Success! New vendor setup done.',
          'type' => 'success',
          'location' => site_url('/third-party/setup-vendor')
        ...
        $data['banks'] = $this->banks->getBanks();
        $username = $this->session->user_username;
        $this->authenticate_user($username, 'pages/payment/setup-vendor', $data);
      }
  ...
  public function getVendors(){
   ...
   // return dd($data);
    $username = $this->session->user_username;
    $this->authenticate_user($username, 'pages/payment/manage-vendors', $data);
  }


    public function getThirdpartyEntry($id){
      $entry = $this->entrypaymentmaster->getEntryMasterById($id);
        $data = [
            'entry'=>$entry,
            'entryDetails'=>$this->entrypaymentdetail->getPaymentDetailsByMasterId($id),
            'verifiedBy'=>$this->user->getUserById($entry->entry_payment_verified_by),
        ];
        $username = $this->session->user_username;
