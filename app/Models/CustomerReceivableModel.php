...
class CustomerReceivableModel extends Model{
    ...
    'cr_approve','cr_approved_by','cr_verified_by','cr_date_approved', 'cr_approve_comment', 'cr_attachment'];
     ...
     public function getVerifiedReceivables(){
            ...
            $builder->join('coop_banks', 'customer_receivables.cr_coop_bank_id = coop_banks.coop_bank_id');
            $builder->join('banks', 'banks.bank_id = coop_banks.bank_id');
            $builder->join('users', 'users.user_id = customer_receivables.cr_verified_by');
            $builder->join('coas', 'coas.glcode = coop_banks.glcode');
            $builder->join('customer_setups', 'customer_receivables.cr_customer_setup_id = customer_setups.customer_setup_id');
    ...
    public function getReceiptEntries(){
      $builder = $this->db->table('customer_receivables');
      $builder->join('coop_banks', 'coop_banks.coop_bank_id = customer_receivables.cr_coop_bank_id');
      $builder->join('banks', 'coop_banks.bank_id = banks.bank_id');
      $builder->join('coas', 'coas.glcode = customer_receivables.cr_gl_cr', 'left');
      $builder->join('customer_setups', 'customer_setups.customer_setup_id = customer_receivables.cr_customer_setup_id');
      $builder->orderBy('customer_receivables.customer_receivable_id', 'DESC');
      return $builder->get()->getResultObject();
    }
